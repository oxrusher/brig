...
class rowset_lite : public brig::rowset {
  ...
  tiles m_iter;
public:
  rowset_lite(size_t cols, int zoom, const brig::boost::box& env, int rows);
...
inline std::vector<std::string> rowset_lite::columns()
{
  ...
  for (size_t i(0); i < m_cols; ++i)
    cols.push_back(layer::column_geometry());
  return cols;
}
...
inline bool rowset_lite::fetch(std::vector<variant>& row)
{
  ...
  const blob_t blob(brig::boost::as_binary(tl.get_mbr()));
  row.resize(m_cols);
  for (size_t i(0); i < m_cols; ++i)
    row[i] = blob;
  return true;
