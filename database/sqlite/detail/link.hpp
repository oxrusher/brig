...
class link : public brig::database::detail::link {
  ...
  void close_stmt();
public:
  link(std::shared_ptr<connection> dbc) : m_dbc(dbc), m_stmt(0), m_done(false)  {}
  ...
  virtual void commit()  { exec("COMMIT", std::vector<variant>(), std::vector<column_detail>()); }
  virtual void rollback()  { exec("ROLLBACK", std::vector<variant>(), std::vector<column_detail>()); }
}; // link
...
inline void link::exec(const std::string& sql, const std::vector<variant>& params, const std::vector<column_detail>& /*param_cols*/)
{
  ...
  {
    close_stmt();
    m_stmt = m_dbc->prepare_stmt(sql);
    m_sql = sql;
  }
  else
  {
    m_dbc->check(lib::singleton().p_sqlite3_reset(m_stmt));
    m_done = false;
  }

  for (size_t i(0); i < params.size(); ++i)
    m_dbc->check(bind(m_stmt, i, params[i]));

  switch (lib::singleton().p_sqlite3_step(m_stmt))
  {
  default: m_dbc->error(); break;
...
inline bool link::fetch(std::vector<variant>& row)
{
  ...
  switch (lib::singleton().p_sqlite3_step(m_stmt))
  {
  default: m_dbc->error(); return false;
