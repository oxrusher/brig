...
inline std::string sql_geometries(DBMS sys, bool order_by) // TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME
{
  ...
  case Ingres: sql = "SELECT g.f_table_schema scm, g.f_table_name tbl, g.f_geometry_column col FROM geometry_columns g JOIN (" + sql_tables(sys, std::string(), false) +") t ON g.f_table_schema = t.scm AND g.f_table_name = t.tbl"; break; // todo:
