...
inline column_type dialect_ms_sql::get_type(const identifier& dbms_type_lcase, int scale)
{
  if (!dbms_type_lcase.schema.empty()) return VoidColumn;
  if (dbms_type_lcase.name.compare("geometry") == 0 || dbms_type_lcase.name.compare("geography") == 0) return Geometry;
  if (dbms_type_lcase.name.compare("bit") == 0) return Integer;
  if (dbms_type_lcase.name.compare("image") == 0) return Blob;
...
{
  if (tbl[col]->dbms_type_lcase.name.compare("geography") == 0) return "";
  return "\
...
inline column_definition dialect_ms_sql::fit_column(const column_definition& col)
{
  ...
  case Blob: res.dbms_type_lcase.name = "varbinary(max)"; break;
  case Double: res.dbms_type_lcase.name = "float"; break;
  case Geometry:
...
inline table_definition dialect_ms_sql::fit_table(const table_definition& tbl, const std::string& schema)
{
  ...
  if (find_if(begin(res.indexes), end(res.indexes), [&](const index_definition& idx){ return Primary == idx.type; }) == end(res.indexes))
  {
    ...
    if (unq_idx == end(res.indexes))
    {
      ...
      col.type = Integer;
      col.dbms_type_lcase.name = "bigint identity";
      col.not_null = true;
...
inline std::string dialect_ms_sql::sql_parameter(const command_traits& trs, const column_definition& param, size_t order)
{
  ...
  if (Geometry == param.type && !trs.writable_geometry) return param.dbms_type_lcase.name + "::STGeomFromWKB(" + marker + ", " + string_cast<char>(param.srid) + ").MakeValid()";
  return marker;
}
...
inline std::string dialect_ms_sql::sql_column(const command_traits& trs, const column_definition& col)
{
  ...
  if (String == col.type && col.dbms_type_lcase.name.find("time") != string::npos) return "CONVERT(CHAR(19), " + id + ", 126) AS " + id;
  if (String == col.type && col.dbms_type_lcase.name.find("date") != string::npos) return "CONVERT(CHAR(10), " + id + ", 126) AS " + id;
...
inline std::string dialect_ms_sql::sql_intersect(const table_definition& tbl, const std::string& col, const boost::box& box)
{
  ...
  const bool geography(col_def->dbms_type_lcase.name.compare("geography") == 0);
  ostringstream stream; stream.imbue(locale::classic()); stream << scientific; stream.precision(17);
