...
inline std::string sql_schema_filter(DBMS sys, const std::string& col)
{
  ...
  {
  default: return "";
  case DB2: return "RTRIM(" + col + ") NOT IN ('APP','DB2GSE','DB2QP','NULLID','SQLJ','ST_INFORMTN_SCHEMA') AND " + col + " NOT LIKE 'SYS%'";
  case Informix: return "RTRIM(" + col + ") NOT IN ('sde')";
  case Ingres: return col + " NOT IN ('$ingres')";
