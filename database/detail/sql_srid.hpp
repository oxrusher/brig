...
inline std::string sql_srid(const DBMS sys, const identifier& tbl, const column_definition & col)
{
  ...
  case Ingres: return "SELECT c.srid, (CASE s.auth_name WHEN 'EPSG' THEN s.auth_srid ELSE NULL END) epsg FROM (SELECT srid FROM geometry_columns WHERE f_table_schema = '" + tbl.schema + "' AND f_table_name = '" + tbl.name + "' AND f_geometry_column = '" + col.name + "') c LEFT JOIN spatial_ref_sys s ON c.srid = s.srid";
