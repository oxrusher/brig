...
QPolygonF get_line(uint8_t byte_order, InputIterator& itr, const frame& fr)
{
  const uint32_t count(get<uint32_t>(byte_order, itr));
  QPolygonF line; line.reserve(count);
  for (uint32_t i(0); i < count; ++i)
    line.push_back( get_point(byte_order, itr, fr) );
  return line;
}
...
{
  painter.drawPolyline( get_line(byte_order, itr, fr) );
}
